...
public class InvoiceServiceLayer {
    ...
    GamestoreCatalogClient client;  //this represents gamestore-catalog repositories.

    @Autowired
    public InvoiceServiceLayer(InvoiceRepository invoiceRepository,ProcessingFeeRepository processingFeeRepository,TaxRepository taxRepository,GamestoreCatalogClient client) {
    ...
    public InvoiceViewModel createInvoice(InvoiceViewModel invoiceViewModel) {
        ...
        if (invoiceViewModel.getItemType().equals(CONSOLE_ITEM_TYPE)) {
            Optional<Console> returnVal = client.getConsole(invoiceViewModel.getItemId());
            Optional<Console> tempCon = client.getConsole(invoiceViewModel.getItemId());

            if (returnVal.isPresent()) {
                tempCon = Optional.of(returnVal.get());
            } else {
                throw new IllegalArgumentException("Requested item is unavailable.");
            }

            if (invoiceViewModel.getQuantity() > invoiceViewModel.getQuantity()){
            ...
            Optional<Game> tempGame = null;
            tempGame = client.getGameById(invoice.getItemId());
            Optional<Game> returnVal = (client.getGameById(invoice.getItemId()));

            if (returnVal.isPresent()) {
                tempGame = Optional.of(returnVal.get());
            } else {
                throw new IllegalArgumentException("Requested item is unavailable.");
            }

            if(invoiceViewModel.getQuantity() >  tempGame.get().getQuantity()){
                throw new IllegalArgumentException("Requested quantity is unavailable.");
            }
            invoice.setUnitPrice(tempGame.get().getPrice());

        } else if (invoiceViewModel.getItemType().equals(TSHIRT_ITEM_TYPE)) {
            TShirt tempTShirt = null;
            Optional<TShirt> returnVal = client.getTShirtById(invoice.getItemId());
